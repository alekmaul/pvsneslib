...
if len(sys.argv) == 1:
    print("Usage: %s <816-tcc asm file>" % sys.argv[0])
    sys.exit(1)

verbose = True
if os.getenv("OPT816_QUIET"):
    verbose = False
...
def is_control(line):
    if len(line) > 0 and line[0] in "jb+-" or line.endswith(":"):
        return True
    return False


def changes_accu(line):
    if (line[2] == "a" and not line[:3] in ["pha", "sta"]) or (
        len(line) == 5 and line.endswith(" a")
    ):
        return True
    else:
        return False
