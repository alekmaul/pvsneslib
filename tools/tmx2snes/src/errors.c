...
void info (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stdout, "%s: ", ERRORBRIGHT("tmx2snes"));
...
void warning (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORPINK("warning") ": ", ERRORBRIGHT("tmx2snes"));
...
void errorcontinue (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORRED("error") ": ", ERRORBRIGHT("tmx2snes"));
  vfprintf (stderr, format, ap);
  va_end (ap);
  fputc ('\n', stderr);
  fflush(stderr);
...
void fatal (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORRED("fatal error") ": ", ERRORBRIGHT("tmx2snes"));
  vfprintf (stderr, format, ap);
  va_end (ap);
  fputc ('\n', stderr);
  exit (EXIT_FAILURE);
