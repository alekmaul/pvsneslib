...
int safe_string_access(char *s, int pos)
{
...
//https://stackoverflow.com/questions/11244626/regular-expression-to-make-sure-that-variable-is-constant
//https://regex101.com/r/kH8xQ5/1
//https://stackoverflow.com/questions/26795591/parsing-vba-const-declarations-with-regex

// Finds a word (i.e. word=="foo" would match on "foo bar" or "a foo bar" but not on "foobar")
unsigned int find_word(char *s, const char *word)
{
	int p = strstr (word);
...
void check_for_const(char *s)
{
	unsigned int i, j;
	const char *staticPrefix = "tccs_";
