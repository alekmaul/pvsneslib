...
void info (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stdout, "%s: ", ERRORBRIGHT("816opt"));
...
void warning (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORPINK("warning") ": ", ERRORBRIGHT("816opt"));
...
void errorcontinue (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORRED("error") ": ", ERRORBRIGHT("816opt"));
  vfprintf (stderr, format, ap);
  va_end (ap);
  fputc ('\n', stderr);
  fflush(stderr);
...
void fatal (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORRED("fatal error") ": ", ERRORBRIGHT("816opt"));
  vfprintf (stderr, format, ap);
  va_end (ap);
  fputc ('\n', stderr);
  exit (EXIT_FAILURE);
