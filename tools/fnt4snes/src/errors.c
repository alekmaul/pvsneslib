...
void info (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stdout, "%s: ", ERRORBRIGHT("fnt4snes"));
...
void warning (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORPINK("warning") ": ", ERRORBRIGHT("fnt4snes"));
...
void errorcontinue (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORRED("error") ": ", ERRORBRIGHT("fnt4snes"));
  vfprintf (stderr, format, ap);
  va_end (ap);
  fputc ('\n', stderr);
  fflush(stderr);
...
void fatal (const char *format, ...)
{
  ...
  va_start (ap, format);
  fprintf (stderr, "%s: " ERRORRED("fatal error") ": ", ERRORBRIGHT("fnt4snes"));
  vfprintf (stderr, format, ap);
  va_end (ap);
  fputc ('\n', stderr);
  exit (EXIT_FAILURE);
