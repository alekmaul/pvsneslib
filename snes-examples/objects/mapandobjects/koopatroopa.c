...
void koopatroopainit(u16 xp, u16 yp, u16 type, u16 minx, u16 maxx) {
	...
	koopatroopaobj->width=16; koopatroopaobj->height=16;
...
void koopatroopaupdate(u8 idx) {
    ...
    koopatroopaox = (u16 *) &(koopatroopaobj->xpos+1);  koopatroopaoy = (u16 *) &(koopatroopaobj->ypos+1);  
    koopatroopax = *koopatroopaox;

    koopatroopaobj->count++;
    if (koopatroopaobj->count >= 3) {                                   // Update anim 20 time per sec
